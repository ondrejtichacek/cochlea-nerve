...
elseif isBin             % Contents of an elementary array, type tested already:
   if ~isempty(Data)     % Engine.update fails for empty input!
      if isnumeric(Data)
         if isreal(Data)
            if correct_real_precision == true
               Data = single(Data);
            end
...
switch OutFormat
   ...
   otherwise
      Error_L('BadOutFormat', ...
         '[Opt.Format] must be: HEX, hex, uint8, double, base32, base64.');
end
...
if isa(Data, 'griddedInterpolant')
    warn_id = 'MATLAB:structOnObject';
    warning('off',warn_id)
    Data = struct(...
        'unashable_data_type', 'griddedInterpolant', ...
        'hashable_converted_data', struct(Data));
    warning('on',warn_id)
end
...
for iOpt = 1:nOpt
   ...
   switch lower(aOpt)
      ...
      case {'double', 'uint8', 'base32', 'short', 'base64'}
...
% remove file name
FuncKey = rmfield(FuncKey,'file');

if isfield(FuncKey, 'within_file_path')
    if strcmp(FuncKey.within_file_path, '__empty_context_function')
        FuncKey.within_file_path = '';
    end
end

if isfield(FuncKey, 'workspace')
    for i = 1:numel(FuncKey.workspace)
        vars = fields(FuncKey.workspace{i});
        for j = 1:numel(vars)
            if isa(FuncKey.workspace{i}.(vars{j}),'struct')
                error(['I would rather not hash anonymous functions referencing structures. ', ...
                    'The anonymous function that caused this is:\n\t%s'], FuncKey.function)
            end
        end
    end
end
...
% ******************************************************************************
function Out = fBase32_enc(In)
% Encode numeric vector of UINT8 values to base32 string.
...
% 5.6,2016 (OT)
%
% Pool = [65:90, 50:55];  % [A:Z, 2:7]
Pool = [97:122, 50:55];  % [a:z, 2:7]
v8   = [128; 64; 32; 16; 8; 4; 2; 1];
v5   = [16, 8, 4, 2, 1];

In  = reshape(In, 1, []);
X   = rem(floor(In(ones(8, 1), :) ./ v8(:, ones(length(In), 1))), 2);
Y   = reshape([X(:); zeros(5 - rem(numel(X), 5), 1)], 5, []);
Out = char(Pool(1 + v5 * Y));
end
